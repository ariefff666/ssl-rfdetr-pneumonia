...
def main(config_path: str) -> None:
    ...
    if ckpt_cfg.get("resume_from"):
        ...
        for k, v in saved_state.items():
            # Jika item berupa Dictionary (seperti student_backbone, teacher_head)
            if isinstance(v, dict):
                for sub_k, sub_v in v.items():
                    clean_sub_k = sub_k.replace("module.", "")
                    # Sediakan dua versi (dengan dan tanpa 'module.') 
                    # PyTorch (strict=False) otomatis akan memilih nama layer yang cocok
                    flat_state[f"{k}.{clean_sub_k}"] = sub_v
                    flat_state[f"{k}.module.{clean_sub_k}"] = sub_v
            
            # Jika item langsung berupa Tensor tunggal (seperti 'center')
            else:
                flat_state[k] = v
                # Penyesuaian khusus untuk DINOv2: sinkronisasi nama loss center
                if k == "center":
                    flat_state["criterion.center"] = v
                    
        print("=> Memuat state_dict yang sudah diratakan (flattened) ke dalam model...")
